...
public class DrugsService {
  ...
  public Drugs saveDrugsDetails(Drugs drugs, MultipartFile[] file) throws CustomException, Exception {
    ...
    drugs.setDrugsId(String.valueOf(this.sequenceGeneratorService.generateSequence(Drugs.SEQUENCE_NAME)));
  ...
  public Set<Photo> uploadImage(MultipartFile[] multipartFiles) throws IOException {
    ...
    for (MultipartFile file : multipartFiles) {
      Photo imageModels1 = new Photo(file.getOriginalFilename(), file.getContentType(), file.getBytes());
  ...
  public Optional<Drugs> getDrugsByID(String drugsId) throws CustomException, Exception {
    ...
    try {
      if (this.drugsRepository.existsByDrugsId(drugsId)) {
        drugs = this.drugsRepository.findById(drugsId);
      } else {
  ...
  public List<Drugs> getDrugsBySupplierName(String supplierName) {
    ...
    return this.drugsRepository.findBySupplierName(supplierName);
  }

  public List<Drugs> getDrugsByName(String drugsName) {

    return this.drugsRepository.findByDrugsName(drugsName);
  }

  public List<Drugs> getDrugsByCategory(String categories) {

    return this.drugsRepository.findBycategories(categories);
  }
  ...
  public Inventory getDrugsWithSupplierDetails(String supplierName) throws CustomException, Exception {
    ...
    try {
      if (this.drugsRepository.existsBySupplierName(supplierName)) {
        List<Drugs> drugs = this.drugsRepository.findBySupplierName(supplierName);
        for (Drugs var : drugs) {
          name = var.getSupplierName();
        }

        Supplier supplier = this.restTemplate.getForObject("http://SUPPLIER-SERVICE/supplier/details/" + name,
            Supplier.class);
  ...
  public Drugs updateDrugsByCart(String username, String drugsId, Drugs drugs) {
    ...
    Cart cart = this.restTemplate
        .getForObject("http://DRUGS-MICROSERVICE/drugs/cart/username/" + drugsId + "/" + username, Cart.class);
    int x = cart.getDrugsQty();

    Drugs drugs1 = this.restTemplate.getForObject("http://DRUGS-MICROSERVICE/drugs/" + drugsId, Drugs.class);
    ...
    this.drugsRepository.save(drugs);
  ...
  public Drugs updateDrugsDetails(Drugs drugs, String drugsId) throws CustomException, Exception {
    ...
    storingVar = this.drugsRepository.save(drugs);
  ...
  public String deleteDrugs(String drugsId) {
    ...
    this.drugsRepository.deleteById(drugsId);
